...
# Inyección de estilos CSS avanzados y limpios para estilización de fuentes, contenedores y regla de impresión PDF
st.markdown("""
    <style>
    ...
    /* Configuración de estilos optimizados para la exportación de impresión PDF */
    @media print {
        .stSidebar, div[data-testid="stSidebarCollapseButton"], button { display: none !important; }
        .main { width: 100% !important; padding: 0 !important; }
        .main-title { color: #000000 !important; font-size: 2rem; }
    }
    </style>
...
# CONFIGURACIÓN UNIVERSAL DE CÁMARA FOTOGRÁFICA (📸 BOTÓN MAESTRO DE PLOTLY VISIBLE)
config_exportacion = {
    'displayModeBar': True,
    'displaylogo': False,
...
# CENTRO DE EXPORTACIÓN NATIVO DE ARCHIVOS EN LA BARRA LATERAL
st.sidebar.markdown("---")
st.sidebar.header("📥 Centro de Descarga de Reportes")
st.sidebar.write("Use el siguiente botón para generar el informe ejecutivo en formato PDF oficial libre de controles de desarrollo.")

# Inyección de script JS para ejecutar la exportación e impresión a PDF sin romper el servidor
if st.sidebar.button("📄 EXPORTAR COMPONENTE A PDF"):
    st.markdown("""
        <script>
            window.print();
        </script>
    """, unsafe_allow_html=True)
    st.sidebar.success("💡 Asistente Activo: Seleccione 'Guardar como PDF' en su ventana de impresión.")
